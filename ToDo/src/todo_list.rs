...
impl<'a> ToDo{
    pub fn print_out_tasks(&self){
        ...
        if self.task_list.len() == 0 {println!("{}", String::from(" You don't have any tasks!").red().bold()); return;};
        for (name, task) in &self.task_list {
            println!(" {}:\n  Description: {}\n  Done: {}\n", name.purple(), task.description.blue(), if task.is_done {String::from("✅")} else {String::from("❌")});
        }
    }

    pub fn add_task(&mut self, new_task: Task){
        cerr::function_error_wrap!(self.task_list.get(&new_task.name), tet, "error");
        match self.task_list.get(&new_task.name) {
            Some(_) => {
                Console::clear();
                cerr::throw_error("The task's already exist!\n");
            }
            None => {
